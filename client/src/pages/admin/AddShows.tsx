...
import type { Movie } from '../../lib/Interfaces/DashbordIntrfaces';
...
const AddShows = () => {
  ...
  const [dateTimeSelection, setDateTimeSelection] = useState<{ [date: string]: string[] }>({});
  const [dateTimeInput, setDateTimeInput] = useState("");
  const [showPrice, setShowPrice] = useState("");

  const fetchNowPlayingMovies = async () => {
    // Fetch now playing movies from the API
    setNowPlayingMovies(dummyShowsData as Movie[]);
  }
  ...
  const handleDateTimeAdd = () => {
    ...
    setDateTimeSelection((prev) => {
      ...
      if (times.includes(time)) return prev; // Prevent duplicate times for the same date
      return {
        ...prev,
        [date]: [...times, time]
      };
  ...
  const handleRemoveDateTime = (date: string, time: string) => {
    setDateTimeSelection((prev) => {
      const filteredTimes = prev[date].filter(t => t !== time);
      if (filteredTimes.length === 0) {
        const { [date]: ignored, ...rest } = prev;// eslint-disable-line @typescript-eslint/no-unused-vars
        return rest;
      }
      return {
        ...prev,
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <div className='mt-8'>
        ...
        <div className='inline-flex items-center gap-2 border border-gray-600 px-3 y-2 rounded-md'>
          <p className='texr-gray-400 text-sm'>{currency}</p>
      ...
      <button className='mt-6 bg-primary text-white px-8 py-2  hover:bg-primary/70 rounded transition-all cursor-pointer'>Add Shows</button>
    </>
  ) : (
    <Loading />
  )
